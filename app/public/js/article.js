import { viewer } from "https://cdn.jsdelivr.net/gh/pardnchiu/PDMarkdownKit@[VERSION]/dist/PDMarkdownKit.js"

let DATA;
document.addEventListener("DOMContentLoaded", async _ => {
    const json_data = "json_data".$;
    const json_data_href = json_data.href;

    await fetch(json_data_href)
        .then(res => res.json())
        .then(data => (json_data.remove(), DATA = data))
        .catch(_ => json_data.remove());
    ...
    const body = new PD({
        ...
        next: function () {
            ...
            //     delay: 0
            // });
            // "blog".$.$sel("section.bl")._child(viewer.body, "section.page".$);
            // viewer.init();

            (function () {
                const domViewer = new viewer({ 
                    pre: DATA.content, 
                    delay: 0,
                    tagPath: "?keyword=",
                    tagTarget: "_blank",
                ...
                "blog".$.$sel("section.body")._child(domViewer.body);
                domViewer.init();
                setTimeout(function () {
                    "PDMDViewer".$.$childAll.filter(e => {
                        const key = $key(10);
                        if (e.nodeName === "H3") {
                            if (e.$pre(1)) e.$pre(1).id = key;
                            else e.$pre(0).id = key;
                            "details.heading".$._child(
                                "a"._(e.innerText)._go("#" + key)
                            );
                        }
                            ...
                            "details.heading".$._child(
                                "a.layer2"._(e.innerText)._go("#" + key)
                            );
                        }
                            ...
                            "details.heading".$._child(
                                "a.layer3"._(e.innerText)._go("#" + key)
                            );
                        }
                    })
                }, 100);
